...
from .serializers import SellHeaderBaseSerializer, SellHeaderUpdateSerializer, SellDetailBaseSerializer
...
class SellHeaderGetListCreateView(APIView):    
    def get(self, request):
        ...
        # List Kosong
        if not sells:
            return Response(status=status.HTTP_404_NOT_FOUND, data={"message": SELL_NOT_FOUND_MESSAGE})
...
class SellDetailView(APIView):    
    ...
    def get(self, request, header_code):
        try:
            ...
            sell_detail = SellDetail.objects.filter(header_code=header_code, is_deleted=False)
